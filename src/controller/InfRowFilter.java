package controller;

import javax.swing.event.ListSelectionEvent;
import javax.swing.event.ListSelectionListener;

import app.MainFrame;
import files.InfAbstractFile;
import view.InfTable;
import view.RightPanel;

public class InfRowFilter implements ListSelectionListener {

	@Override
	public void valueChanged(ListSelectionEvent e) {
//		RightPanel rightPanel = MainFrame.getInstance().getCurrentRightPanel();
//		InfTable table = (InfTable) rightPanel.getEntityContainer().getTable();
//		((InfAbstractFile) rightPanel.getEntityContainer().getEntity().getInfFile()).filter(table.getSelectedRow());

	}

}
